...
//       case Some(user) =>  Logger.info(user.username.getOrElse("empty"))
//         val secured = SecuredRequest(user, request.authenticator.getOrElse(throw new IllegalStateException()), request)
//         block(secured).map { r =>
//           r
//         }
//       case None => Logger.info("BaseController: user is None, the cookie problem")
//         val user = User(
//           id = UUID.randomUUID(),
//           username = None,
//           profiles = Nil,
...
//         )
//         // val secured = SecuredRequest(user, request.authenticator.get, request)
//         // block(secured).map { r =>
//         //   r
//         // }
//         for {
