...
object ProfileQueries extends BaseQueries[CommonSocialProfile] {
  ...
  case class UpdateProfile(p: CommonSocialProfile) extends Statement {
    override val sql = s"update $tableName set email = ?, first_name = ?, last_name = ?, full_name = ?, avatar_url = ?, created = ? where provider = ? and key = ?"
    override val values = Seq(
    p.email, p.firstName, p.lastName, p.fullName, p.avatarURL, new LocalDateTime(), p.loginInfo.providerID, p.loginInfo.providerKey)
  }
